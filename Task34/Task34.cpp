...
bool diffNum(char ar[], const int size); // Для проверки цифр на "разность" в "загаданном" числе
...
// Функция для проверки массива на наличие одинаковых цифр
bool diffNum(char ar[], int size)
{
    int count = 0;
    for (int i = 0; i < size; i++)
